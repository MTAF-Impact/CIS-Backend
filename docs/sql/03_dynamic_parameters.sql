...
-- store the Admin Setting page reads and writes.
--
...
--
--   * the ~30 detector parameters - they live in cis_detector_settings as
--     typed columns, because two of their constraints are cross-field and a
--     flat key/value setter cannot check them;
...
--   * scoring.harm_weight_policy_disruption  hard ceiling of 0.25 - a bias
--     guardrail against scoring criticism of a government's own policy
--     as harm
...
-- Alerting & risk threshold (operations tier)
  ('alert_threshold', '70', 'number',
   'AP-16 — The FinalClaimScore at or above which a claim reads as Over Threshold on the Alert page, counts as above-threshold on the Overview, and can raise a threshold-crossing notification.'),

-- Overview page display (operations tier)
  ('overview.top_policy_limit', '5', 'number',
   'How many policies the Overview''s leaderboard lists.'),
...
-- Public Policy Bank (operations tier)
  ('policy.upload_warn_size_mb', '50', 'number',
   'AP-17 — File size above which the Add Public Policy modal warns the uploader.'),

-- AI content generation (operations tier)
  ('ai.debunk_segment_max_count', '3', 'number',
   'AP-21 — How many audience-segmented debunk drafts the AI generates for each claim.'),

-- City & locale (operations tier)
  ('monitored_city', 'Jakarta', 'string',
   'AP-22 — The single Indonesian city this instance monitors. Scopes every metric on the Overview page.'),
  ('city_timezone', 'Asia/Jakarta', 'string',
   'IANA zone for the city-local half of every report footer timestamp.'),

-- Claim score — composite weights (analytics tier)
  ('scoring.weight_reach', '0.15', 'number',
   'AP-01 — Share of the composite score contributed by how far the claim has travelled.'),
  ('scoring.weight_velocity', '0.15', 'number',
   'AP-02 — Share contributed by how fast the claim is currently growing.'),
  ('scoring.weight_falseness', '0.30', 'number',
   'AP-03 — Share contributed by how confidently the claim is confirmed false.'),
  ('scoring.weight_harm', '0.30', 'number',
   'AP-04 — Share contributed by the estimated real-world damage the claim could cause.'),
  ('scoring.weight_emotional_intensity', '0.10', 'number',
   'AP-05 — Share contributed by how angry the public reaction to the claim is.'),

-- Harm severity — sub-weights (analytics tier)
  ('scoring.harm_weight_public_safety', '0.35', 'number',
   'AP-06 — Share of Harm Severity carried by risk to physical safety.'),
  ('scoring.harm_weight_institutional_trust', '0.30', 'number',
   'AP-07 — Share carried by erosion of trust in public institutions.'),
  ('scoring.harm_weight_economic', '0.20', 'number',
   'AP-08 — Share carried by economic damage.'),
  ('scoring.harm_weight_policy_disruption', '0.15', 'number',
   'AP-09 — Share carried by concrete interference with policy execution.'),

-- Reach & velocity normalisation (analytics tier)
  ('scoring.reach_normalization_window_days', '90', 'number',
   'AP-10 — How far back R_min/R_max are observed before raw reach is normalised onto 0-100.'),
  ('scoring.reach_weight_impressions', '0.25', 'number',
   'Weight of log(1+Impressions) inside raw reach.'),
  ('scoring.reach_weight_unique_authors', '0.25', 'number',
   'Weight of log(1+UniqueAuthors) inside raw reach.'),
  ('scoring.reach_weight_content_count', '0.25', 'number',
   'Weight of log(1+ContentCount) inside raw reach.'),
  ...
  ('scoring.velocity_interval_hours', '6', 'number',
   'AP-11 — The gap between the two volume readings whose difference is the growth rate.'),
  ('scoring.velocity_zscore_min', '-3', 'number',
   'AP-12 — Standard deviations below baseline that map to a Velocity of 0.'),
  ('scoring.velocity_zscore_max', '3', 'number',
   'AP-12 — Standard deviations above baseline that map to a Velocity of 100.'),
  ('scoring.velocity_epsilon', '0.0001', 'number',
   'AP-13 — Division-by-zero guard for a brand-new claim with no prior volume.'),

-- Public pushback discount (analytics tier)
  ('scoring.npr_window_hours', '36', 'number',
   'AP-14 — The window over which supporting and opposing volume are compared to compute NPR.'),
  ('scoring.discount_gamma', '0.5', 'number',
   'AP-15 — The largest share of a claim''s score that organic pushback can remove.'),
...
-- Climate Sentiment Index (analytics tier)
  ('csi.weight_bcs', '0.5', 'number',
   'AP-18 — Share of the index carried by the overall tone of the climate conversation.'),
  ('csi.weight_risk_load', '0.5', 'number',
   'AP-19 — Share carried by the burden of serious claims on the conversation.'),
  ('csi.window_days', '7', 'number',
   'The rolling average behind the headline gauge figure.'),
  ...
  ('csi.band_risky_ceiling', '33.33', 'number',
   'Index value below which the gauge shows red.'),
...
-- Overview ranking formula (analytics tier)
  ...
  ('overview.treemap_weight_avg_score', '0.5', 'number',
   'Share driven by the average score of its claims.'),
...
--
-- Who changed what, newest first (the audit trail, shared with the
-- detector parameters):
--
