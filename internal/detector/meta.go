...
//
// The standing disclaimer must appear VERBATIM on every report and on the
// network detail page. A requirement to reproduce a paragraph exactly, in two
// renderers, cannot be met by two copies of the paragraph. The same reasoning
// extends to the per-signal method sentences, shown in both the UI panel and
// the report, and to the known limitations, which are stated explicitly
...
// Disclaimer is the standing disclaimer, reproduced verbatim.
//
// Required on EVERY report and on the network detail page. On the report it
// appears twice by design — in full on the cover page, so it cannot be
// missed, and again as the closing "Limitations and disclaimer" section. Do
// not deduplicate those two.
//
...
// Pairwise signal family keys, as recorded in
...
type SignalMeta struct {
	Code string
	Name string
	// Method is the one-sentence plain-language description shown in the UI
	// panel: a policy reviewer must be able to read it without knowing what
	// conductance is, so none of these sentences uses a term it does not
	// immediately explain.
	Method string
	// Weight is the metric's share of the composite.
	Weight float64
	...
	// role in SignalBreadth contested.
	Families []string
}
...
// KnownLimitations are caveats stated explicitly rather than left for a
// reader to infer.
//
// The first: accounts in the same timezone reacting to the same news event
// within the same minute show elevated synchrony. It belongs beside the
// signal scores in the UI and in the report's methodology appendix.
//
// The last is platform-coverage asymmetry. It is carried here because an
// empty network list on a city whose discourse runs through closed messaging
// channels is not evidence of no coordination, and reading it as reassurance
// is a specific, foreseeable misuse of this feature.
...
//
// The truncation is displayed, not merely recorded: a truncated run has known
// incomplete recall, and the analyst has to be told at the point of judgement
// rather than left to find it in run metadata.
...
// ConfidenceCapNote explains why a confidence cap applied.
...
// BreadthGuardNote explains the signal-breadth guard, carried wherever a band is shown.
...
// AvailabilityPublic and AvailabilityDeleted are the two labels rendered
...
// PrecisionTarget is the recommended operational target: precision above
// 0.85 on analyst-confirmed networks, on a rolling 90-day basis.
//
// Recall is deliberately secondary: a missed network costs a missed
...
// GraphLegibilityLimit is the graph's node ceiling. Beyond it the graph is
// rendered as its k-core and the reduction is stated.
