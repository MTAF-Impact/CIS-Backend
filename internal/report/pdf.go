...
// CIS palette (PRD 5.1). The report is a government document rendered in the
// product's own colours, and PRD 10.8 names two of them specifically: Regal
// Navy headers and Sea Green section rules.
var (
...
// A4 portrait geometry in millimetres (PRD 10.8).
const (
	pageWidth   = 210.0
	marginLeft  = 18.0
	marginRight = 18.0
...
// Render produces the ten-section PDF of PRD 10.8.
...
// footerFunc renders PRD 10.8's per-page footer: report ID, network ID, page
// number, and the generation timestamp in UTC AND city-local time.
//
...
// renderCover is PRD 10.8 item 1.
//
// The standing disclaimer is printed IN FULL on the cover, "not buried at the
// back". That placement is a requirement, and it is the single most important
// piece of typography in the document: a reader who opens this file and reads
// nothing else must still learn that it is not a determination that any account
// is automated.
func renderCover(pdf *fpdf.Fpdf, d Data) {
	...
	// appendix. PRD 10.5.1 requires the caveat at the point of judgement, and
	// for a document that will be read once by a stranger, that point is here.
...
// renderExecutiveSummary is PRD 10.8 item 2: at most 200 words, plain language,
// from a fixed template with numeric slots.
//
// Deliberately not generated by a model. The requirement is that "the same
// detection always produces the same summary and the summary can never assert
// more than the data supports" — both properties fail the moment a language
// model is in the loop, and the second is the one that matters.
...
// renderDetectionBasis is PRD 10.8 item 3: the full signal breakdown with raw
// counts, SignalBreadth, the confidence rule applied, the unavailable families,
// and the claim-relevance figures.
func renderDetectionBasis(pdf *fpdf.Fpdf, d Data) {
	...
	// Stated as an explicit list, never silently omitted. PRD 10.5.2.5: the
	// pipeline degrades gracefully when a family is unavailable, "and the run
	// metadata records the omission so the report can state which signals were
	...
	// A recurrence inherits history but not relevance (PRD 10.5.1), and both
	// the detail page and the report must state the current primary claim AND
	// the prior anchoring claims. That sentence — "this same set of accounts
	// previously amplified claims X and Y" — is what makes a referral
	// actionable, so it belongs in the detection basis rather than in a footnote.
...
func signalBlock(pdf *fpdf.Fpdf, sig dto.SignalDetail) {
	...
	// The meter. A bar is not decoration here: US50 asks for one so a reader
	// can compare five numbers without arithmetic.
	meterY := pdf.GetY() + 0.5
...
// renderActivityTimeline is PRD 10.8 item 4: the burst chart with anomalous
// bins annotated by z-score and the bin width stated.
...
// renderNetworkStructure is PRD 10.8 item 5: the graph figure from the STORED
// layout, plus internal density, conductance and the comparison-account count.
//
...
// renderRepresentativeContent is PRD 10.8 item 6: the top duplicate groups with
// canonical text, variant count, and a table of variants with handle, timestamp
// to the second, and text, with shared spans highlighted.
...
// same in the report. PDF has no <mark>, so the highlight is rendered as
// bracketing — the point is that a reader can see WHICH part is common, and
// bracketing carries that in plain text, in the CSV, and read aloud.
...
// renderAccountAnnex is PRD 10.8 item 7: the full member table, one row per
// account, with US55's columns.
//
// Mandatory in a platform referral and non-toggleable, because US59 says a
// referral without the account list is not actionable.
...
// renderInternalContext is US59's internal-briefing-only material: analyst
// notes, review history, and the linked claim and policy context.
//
// Never rendered in a platform referral. US59 draws that line so one detection
// can serve both audiences without the referral carrying the city's internal
// deliberation into a third party's hands.
...
// renderMethodology is PRD 10.8 item 8: every parameter value used, model and
// library versions, the random seed, the null-model settings, normalisation
// rules, and all exclusions applied.
//
// "This section is what makes the report reproducible, and reproducibility is
...
// renderLimitations is PRD 10.8 item 9: Section 10.9.2, in full.
//
...
// renderChainOfCustody is PRD 10.8 item 10: evidence snapshot ID, snapshot
// hash, detection run ID, and the export audit entry ID.
//
...
// capWords enforces PRD 10.8 item 2's 200-word ceiling on the executive summary.
