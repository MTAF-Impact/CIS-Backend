...
// Asks the AI service to run the Coordinated-Network Detector.
//
// # Why this is a hand-off and not a synchronous computation
//
// A full run can take minutes, far beyond any HTTP request this backend
// should hold open. So the call announces the run, the AI service acknowledges
// in milliseconds, and the work happens in its background — the same
// hand-off shape used elsewhere for long-running matching work.
//
...
//
// Changing a parameter must never retroactively alter a stored detection:
// every run records the parameter set in force at execution time. Sending
// them with the request is what makes that true even if an admin edits the
// configuration while the run is in flight.

// pathDetectionRun starts a Coordinated-Network Detector run.
const pathDetectionRun = "/api/v1/detection/runs"

// pathDetectionPurge asks the AI service to purge evidence snapshots that
// have passed their retention date.
//
...
type DetectionRunRequest struct {
	...
	// TriggerSource is scheduled | velocity | on_demand, recorded on the run so
	// an analyst can tell an automated detection from one somebody asked for.
	TriggerSource string `json:"trigger_source"`
	// WindowStart and WindowEnd are computed by the backend from W, so that
	// consecutive scheduled runs overlap by 50% of the window. Leaving the
	// window to the AI service would put that rule in the one place the
...
// because a snapshot must be retained as long as any report was generated
// from it, and cis_network_reports is where that is recorded. A blanket TTL
// delete would
