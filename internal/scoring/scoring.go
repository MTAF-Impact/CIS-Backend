// Package scoring encodes the Claim Scoring System as it is presented by this
// API.
//
...
//
//   - the configured weights, so the UI can explain the ranking;
//   - defensive bounds checks that still assert the 0-100 bound even though the
//     formulas are mathematically guaranteed to stay in range;
//   - the dormancy rules that decide when NPR and the discount must be
//     presented as not-applicable rather than as a number.
//
// # Weights are values, not constants
//
// The weights below are the package's defaults, and they are what it falls
// back to. The live values are admin-configurable and stored in cis_settings
// (see models.ConfigParams), so every function that needs them takes them as
// an argument rather than reading a package constant. That is what stops the
// API explaining a ranking with one set of numbers while the AI service
// computed it with another.
package scoring
...
// Discount bounds. With the default gamma of 0.5, even total pushback
// reduces a score by at most half.
const (
...
// ClampRatio constrains NPR to its native [0,1] range.
...
// ClampDiscount constrains DiscountFactor to its floor and 1.
//
...
// every breakdown so the ranking is explainable in the UI.
type Weights struct {
...
// HarmWeights describes the Harm Severity sub-weights.
...
// FormulaSummary is the plain-language explanation behind the info-tooltip on
...
// drift apart — including after an admin retunes them.
...
// claim. A dormant claim has no supporting or opposing volume in the rolling
// window, so it is flagged rather than discounted — its priority must never be
// lowered on the basis of missing data.
const DormancyNote = "No supporting or opposing volume in the rolling window, so this claim is flagged dormant " +
	"rather than discounted. NPR and DiscountFactor are not applicable."
